document.addEventListener('DOMContentLoaded', (event) => {
  ...
  const textures = {};
  // LOAD SPRITESHEET
  const assetsToLoader = ['resources/ken.json', 'resources/shoryuken.json', 'resources/tatsumaki.json'];
  const loader = new PIXI.AssetLoader(assetsToLoader);
  loader.onComplete = onAssetsLoaded;
  loader.load();
  // CREATE STAGE
  const stage = new PIXI.Stage(0x000000, true);
  ...
  function onAssetsLoaded() {
    ...
    const texture = PIXI.Texture.fromImage('resources/background.png');
    const bgk = new PIXI.Sprite(texture);
    bgk.position.x = 0;
    bgk.position.y = 0;
    stage.addChild(bgk);
    //---------------------------------
