...
// //////////////////////////////////////////////
const WW = app.view.width;
const HH = app.view.height;
// //////////////////////////////////////////////
const screenLoading = new PIXI.Container();
...
loadingBar.drawRect(0, (HH / 2), 1, 20);

loadingBar.position.x = (WW / 2) - (loadingBar.width / 2);
// loadingBar.lineStyle(5, 0xFF0000);
app.stage.addChild(screenLoading);
app.stage.addChild(screenLoading);
screenLoading.addChild(loadingBar);
// //////////////////////////////////////////////
loader
  // json
...
// //////////////////////////////////////////////
function initScreenLoading() {
  TweenMax.to(screenLoading, 0.3, {
    width: 0,
  });
...
function initScreenMain() {
  app.stage.removeChild(screenGuide);
...
function initScreenGuide() {
  ...
  const ken_ForwradJumpKick_L = new PIXI.AnimatedSprite(ForwradJumpKick_L_sheet.animations
    .ForwradJumpKick_L);
  ...
  const ken_ForwradJumpPunch_L = new PIXI.AnimatedSprite(ForwradJumpPunch_L_sheet.animations
    .ForwradJumpPunch_L);
  ...
  const ken_hadoukan_air_ending = new PIXI.AnimatedSprite(hadoukan_air_ending_sheet.animations
    .hadoukan_air_ending);
  ...
  const ken_fireShouryuken = new PIXI.AnimatedSprite(fireShouryuken_sheet.animations
    .fireShouryuken);
  ...
  const ken_spinningBackroll = new PIXI.AnimatedSprite(spinningBackroll_sheet.animations
    .spinningBackroll);
  ...
  button.position.x = 0;
  button.position.y = HH - 100;
  button.interactive = true; // 設定可以互動
  button.buttonMode = true; // 當滑鼠滑過時顯示為手指圖示
  button.click = function () {
    initScreenMain();
  };
  const text = new PIXI.Text('next frame');
  text.anchor.set(0.5);
  ...
  screenGuide.addChild(button);
  console.log(button);

  TweenMax.to(screenGuide, 1, {
    alpha: 1,
  });
}
